...
class ReleaseCommand extends AbstractCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            // $this->git->isBranch($branch->name)->isRemote(Config::REMOTE);
    ...
    private function commitRelease(): void
    {
        $this->processHelper->run($this->output, Process::fromShellCommandline('git add .'));
        $statusCommand = Process::fromShellCommandline('git status -s');

        $this->processHelper->run($this->output, $statusCommand, null, function (string $type, string $data): void {
            if (Process::ERR === $type) {
